...
import com.github.vincentvangestel.rinsimextension.experiment.ExperimentRunner.AuctionStats;
import com.github.vincentvangestel.rinsimextension.experiment.ExperimentRunner.ExperimentInfo;
...
abstract class ResultWriter implements ResultListener {
  ...
  static void appendTimeLogSummary(SimulationResult sr, File target) {
    if (sr.getResultObject() instanceof ExperimentRunner.ExperimentInfo) {
      final ExperimentRunner.ExperimentInfo info =
        (ExperimentRunner.ExperimentInfo) sr.getResultObject();
